...
def create_outputs(outputs,occlusion_rate,frame_path,label,track_id,x1,y1,x2,y2,x_center,y_center, distance):
    if occlusion_rate == 100:
        outputs.append((frame_path.name, label, "id:", track_id, "dist:", distance, "pos:", x1, y1, x2, y2, "{x_center:.2f}", y_center, "o:", 3))  # Not visible at all
    elif 50 <= occlusion_rate < 100:
        outputs.append((frame_path.name, label, "id:", track_id, "dist:", distance, "pos:", x1, y1, x2, y2,  "{x_center:.2f}", y_center, "o:", 2))  # Partially visible
    elif 0 < occlusion_rate < 50:
        outputs.append((frame_path.name, label, "id:", track_id, "dist:", distance, "pos:", x1, y1, x2, y2, "{x_center:.2f}", y_center, "o:", 1))  # Mostly visible
    else:
        outputs.append((frame_path.name, label, "id:", track_id, "dist:", distance, "pos:", x1, y1, x2, y2,  "{x_center:.2f}", y_center, "o:", 0))  # Totally visible
    return outputs
